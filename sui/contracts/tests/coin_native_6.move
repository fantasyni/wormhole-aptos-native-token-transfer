#[test_only]
module wormhole_ntt::coin_native_6 {
    use std::option::{Self};
    ...
    fun init(coin_witness: COIN_NATIVE_6, ctx: &mut TxContext) {
        let (
            treasury_cap,
            coin_metadata
        ) =
    ...
    public fun take_treasury_cap(
        scenario: &Scenario
    ): TreasuryCap<COIN_NATIVE_6> {
        test_scenario::take_shared(scenario)
    }

    public fun return_treasury_cap(
        treasury_cap: TreasuryCap<COIN_NATIVE_6>
    ) {
