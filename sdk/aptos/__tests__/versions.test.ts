...
import { AptosNtt } from "../src/index.js";
...
describe("ABI Versions Test", function () {
  ...
  test("It initializes from Rpc", async function () {
    const ntt = await AptosNtt.fromRpc(await ctx.getRpc(), {
      Sepolia: {
        ...ctx.config,
  ...
  test("It initializes from constructor", async function () {
    const ntt = new AptosNtt("Testnet", "Sepolia", await ctx.getRpc(), {
  ...
  test("It gets the correct version", async function () {
    ...
    const version = await AptosNtt._getVersion(manager, await ctx.getRpc());
